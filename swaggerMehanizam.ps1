...
if ($allResultsByServer.Count -gt 0) {
    ...
    #$htmlFilePath = "\\fsi\ORG\ICT\DevOps\Email-Sender\SwaggerProvera.html"    
  ...
  <#  Send-Email -mailParam @{
        From       = "SwaggerProvera-$SufiksZaMail@TFS-TEST.rs"
        To         = 'ict_cmd@bancaintesa.rs'
        Subject    = "Provera Check Metoda - $SufiksZaMail"
        Body       = "\\fsi\ORG\ICT\DevOps\Email-Sender\SwaggerProvera.html"
        SmtpServer = 'smtpq'
    }
    #>
}
