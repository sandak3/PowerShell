...
if ($env:RELEASE_ENVIRONMENTNAME -eq 'Test') {
    $servers = @{
        ComputerName = 'Test11', 'Test12', 'Test13', 'Test14'
    }
    $SufiksZaMail = 'Test' # Definisanje sufiksa naslova maila radi lakseg filtriranja za RULE 
}
elseif ($env:RELEASE_ENVIRONMENTNAME -eq 'Test2') {
    $servers = @{
        ComputerName =  'Test22', 'Test03'
    }
    $SufiksZaMail = 'Test2' # Definisanje sufiksa naslova maila radi lakseg filtriranja za RULE
}
elseif ($env:RELEASE_ENVIRONMENTNAME -eq 'DEV') {
    Write-Host "Ne radimo i dalje dev"
}
else {
    Write-Host "Nesto nije u redu"
}

$fileContent = Get-Content "C:\Powershell\SwaggerProvera\Exceptions.txt"
...
$SB = {    
    ...
    $excludedServiceCombinations = @(
        @{ Server = "Test11"; Service = "testApi", "SandaApi", "Shop.Api" },
        @{ Server = "Test12"; Service = "testApi", "SandaApi", "Shop.Api" },
        @{ Server = "Test13"; Service = "testApi", "SandaApi", "Shop.Api"}
        @{ Server = "Test22"; Service = "testApi", "SandaApi", "Shop.Api" }
        @{ Server = "Test03"; Service = "DJS.Api" }
    )
...
$outputPathRmReporting = "\\share\dev\SwaggerProvera_$stage.html"
