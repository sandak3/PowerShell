    ...
    $stage = $env:Okruzenje
    if($stage -eq 'MNT'){
    $serverListPath = "C:\Powershell\Releases\Task Schedulers\OtvoreneTransakcije\ServerListMNT.txt"
    }
    if($stage -eq 'FC2'){
    $serverListPath = "C:\Powershell\Releases\Task Schedulers\OtvoreneTransakcije\ServerListFC2.txt"
    }
            ...
            if ($null -eq $result) {
                ...
                foreach ($tran in $result) {
                    ...
                    if ($razlika -gt 10) {
                        ...
                        # Provera da li login počinje sa 'ICT-025, itg, fin' i ubijanje sesije ako je true
                        $isKilled = $false
                        if ($tran.'HOST NAME' -match "^ICT-025|^itg|^fin") {
                            try {
                                Write-Host "##vso[task.logissue type=warning]Ubijanje sesije za ICT-025, itg, fin korisnika: $($tran.'HOST NAME') na serveru $server"
...
"@
    ...
    $outputPathRmReporting = "\\fsi\ORG\ICT\DevOps\RM_Reporting\OtvoreneTransakcije_$stage.html"
    ...
    if ($zastava -eq 1) {
     ...
     <#   Send-Email -mailParam @{
            From       = "$PipNaziv@$PipAgent.rs"
            To         = 'ict_cmd@bancaintesa.rs'
            Subject    = 'Otvorene Transakcije[MNT, FC2]'
            Body       = "\\fsi\ORG\ICT\DevOps\Email-Sender\transakcije.html"
            SmtpServer = 'smtpq'
        }
     #>
    }
